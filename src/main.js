...
// Wrap the entire script in a DOMContentLoaded listener to prevent race conditions
...
function initialize() {
  ...
  const heroScene = new WebGLScene('webgl-canvas');
  heroScene.init('particles');
  ...
  window.addEventListener('mousemove', (e) => {
    ...
    heroScene.updateMousePosition(x, y);
    aboutScene.updateMousePosition(x, y);
  });
  ...
  cards.forEach((card, index) => {
    ...
    card.querySelectorAll('.card-front, .card-back').forEach(face => {
  ...
  gsap.matchMedia().add({
    ...
    .to(cards, {
      x: (i) => {
        ...
        if (isDesktop) return centerOffset * 280; // Tighter spread for desktop
        if (isTablet) return centerOffset * 220;
        return centerOffset * 80; // Closer spread for mobile
      },
  ...
  ScrollTrigger.create({
    ...
    scrub: 1.5,
    onUpdate: (self) => {
      if (finalSequence && finalSequence.update) {
        finalSequence.update(self.progress);
      }
    },
  });
  ...
  .to(".final-sequence-endcard", {
      onStart: () => { document.querySelector('.final-sequence-endcard').classList.add('is-visible'); },
      onReverseComplete: () => { document.querySelector('.final-sequence-endcard').classList.remove('is-visible'); }
   }, ">");
